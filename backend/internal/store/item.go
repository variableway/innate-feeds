...
func (s *Store) UpdateItemEmbedding(id int64, embedding []float32) error {
	...
	// Mirror to the native vector column when supported. This is best-effort:
	// if the column is missing or the dim mismatches, the in-Go brute-force
	// path still works on the BLOB.
	if s.vectorReady {
		vecStr := encodePgVector(embedding)
		if _, derr := s.db.Exec(
			`UPDATE items SET embedding_vec = $1::vector WHERE id = $2`,
			vecStr, id,
		); derr != nil {
			// Log but don't fail; brute-force is still available.
			slog.Default().Warn("failed to update embedding_vec", "id", id, "error", derr)
		}
	}
	return nil
}
...
// It uses the native pgvector operator when available, otherwise falls
// back to an in-Go brute-force scan over the BLOB embedding column.
func (s *Store) SearchItemsSemantic(queryVec []float32, limit int) ([]*SearchItemResult, error) {
	...
	if s.vectorReady {
		return s.searchItemsSemanticPgVector(queryVec, limit)
	}
	return s.searchItemsSemanticBruteForce(queryVec, limit)
}

// searchItemsSemanticPgVector delegates to PostgreSQL + pgvector.
// The <=> operator returns cosine distance (1 - cosine_similarity).
func (s *Store) searchItemsSemanticPgVector(queryVec []float32, limit int) ([]*SearchItemResult, error) {
	vecStr := encodePgVector(queryVec)
	rows, err := s.db.Query(`
	...
	for rows.Next() {
		r := &SearchItemResult{}
		var score float64
		if err := rows.Scan(&r.ID, &r.FeedID, &r.Title, &r.PubDate, &score); err != nil {
			continue
		}
...
// searchItemsSemanticBruteForce is the in-Go fallback. It reads every
// embedding from the BLOB column, computes cosine similarity, and ranks
// in memory. This works on any backend but does not scale.
func (s *Store) searchItemsSemanticBruteForce(queryVec []float32, limit int) ([]*SearchItemResult, error) {
	rows, err := s.db.Query(`
...
// encodePgVector formats a float32 slice as the pgvector text format
// "v1,v2,v3" (no brackets). pgx binds the string and pgvector parses it.
func encodePgVector(v []float32) string {
	...
	for i, f := range v {
		...
		// Use enough precision for cosine similarity (~1e-7 is plenty).
