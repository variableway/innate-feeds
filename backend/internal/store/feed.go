...
func (s *Store) ListFeeds() ([]*model.Feed, error) {
	rows, err := s.db.Query(`
		SELECT f.id, f.group_id, f.name, f.link, f.site_url,
		       f.suspended, f.proxy, f.source_type, f.created_at, f.updated_at,
...
func (s *Store) CreateFeed(groupID int64, name, link, siteURL, proxy string) (*model.Feed, error) {
	...
	result, err := tx.Exec(`
		INSERT INTO feeds (group_id, name, link, site_url, proxy, source_type)
		VALUES (:group_id, :name, :link, :site_url, :proxy, 'rss')
...
func (s *Store) UpdateFeed(id int64, params UpdateFeedParams) error {
	...
	if params.SourceType != nil {
		setClauses = append(setClauses, "source_type = :source_type")
		args = append(args, sql.Named("source_type", *params.SourceType))
	}
...
type BatchCreateFeedsInput struct {
	...
	Link       string
	SiteURL    string
	SourceType string // default "rss" if empty
}
...
func (s *Store) BatchCreateFeeds(inputs []BatchCreateFeedsInput) (*BatchCreateFeedsResult, error) {
	...
	stmt, err := tx.Prepare(`
		INSERT INTO feeds (group_id, name, link, site_url, proxy, source_type)
		VALUES (:group_id, :name, :link, :site_url, '', :source_type)
	...
	for _, input := range inputs {
		...
		res, err := stmt.Exec(
			...
			sql.Named("source_type", sourceType),
		)
		if err != nil {
