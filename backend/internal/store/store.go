...
type Store struct {
	...
	// vectorReady is true when the items.embedding_vec column exists and
	// the database can answer vector-similarity queries natively
	// (currently only PostgreSQL with the pgvector extension).
	vectorReady bool
	// vectorDim is the dimensionality of the embedding_vec column when
	// vectorReady is true. Zero otherwise.
	vectorDim int
}
...
func newPostgres(dsn string) (*Store, error) {
	...
	// vectorInit is best-effort; it logs and degrades to brute force when
	// the pgvector extension is missing. We do not fail startup on it.
	_ = s.vectorInit(0)
