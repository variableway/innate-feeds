...
func (h *Handler) SetupRouter() *gin.Engine {
	...
	// Public feed output — RSS/Atom/JSON Feed for any RSS reader.
	// Unauthenticated by design: these endpoints are consumed by external readers.
	// HEAD is supported alongside GET so curl -I, fetch HEAD, and feed
	// validators that probe for changes work without burning bandwidth.
	feedOutputRoutes := []struct {
		path string
		fn   gin.HandlerFunc
	}{
		{"/feeds/:id/rss.xml", h.feedRSS},
		{"/feeds/:id/atom.xml", h.feedAtom},
		{"/feeds/:id/feed.json", h.feedJSON},
		{"/groups/:id/rss.xml", h.groupRSS},
		{"/groups/:id/atom.xml", h.groupAtom},
		{"/groups/:id/feed.json", h.groupJSON},
		{"/all/rss.xml", h.allRSS},
		{"/all/atom.xml", h.allAtom},
		{"/all/feed.json", h.allJSON},
	}
	for _, route := range feedOutputRoutes {
		r.GET(route.path, route.fn)
		r.HEAD(route.path, route.fn)
	}
