...
func (h *Handler) isSessionValid(sessionID string) bool {
	ok, err := h.store.GetSession(sessionID)
	return ok && err == nil
}

// createSession generates a new session ID, stores it persistently, and sets the session cookie.
func (h *Handler) createSession(c *gin.Context) {
	now := time.Now()
	expiresAt := now.Add(sessionTTL)
	sessionID := uuid.New().String()

	_ = h.store.CreateSession(sessionID, expiresAt)
