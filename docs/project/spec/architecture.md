...
前端 (Vite + React) ──/api/*──► Hono 后端 ──► SQLite (feeds.db)
     ...
     └── 静态模式 (VITE_STATIC_MODE) └── gh CLI / Firecrawl 采集
              └── manifest + JSON chunks + digest.json
              └── 浏览器仍可直连 GitHub（digest / README live）
```
...
| — | 快照 + live GitHub | Pages 有离线/限流回退；打开详情仍刷新 |
| — | 90 日 window sync | digest / 新 starred README 预取；trending 只能抓「当前」榜 |
...
| 采集 | `backend/src/collector/` | github、firecrawl、sync、sync-window、issues-digest |
